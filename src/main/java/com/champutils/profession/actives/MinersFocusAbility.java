...
public class MinersFocusAbility implements ProfessionActiveAbility {
    ...
    private static final double DEFAULT_BOOST_PERCENT = 50.0D;
    ...
    public boolean use(ServerPlayer player, ItemStack stack) {
        ...
        double boostPercent = getBoostPercent(stack);
        double multiplier = 1.0D + Math.max(0.0D, boostPercent) / 100.0D;

        ActiveEffectManager.activateTimedWithMultiplier(player, "miners_focus", "Miner's Focus", seconds, stack, multiplier);

        int roundedBoost = (int) Math.round(boostPercent);
        player.sendSystemMessage(Component.literal("§6Miner's Focus active: §fMining passive chances are boosted by §e" + roundedBoost + "% §ffor §e" + secondsText + "s§f."));
        player.displayClientMessage(Component.literal("§6Miner's Focus: +" + roundedBoost + "% passive chance for " + secondsText + "s"), true);
    ...
    private double getDurationSeconds(ServerPlayer player, ItemStack stack) {
        double rolledSeconds = ProfessionToolUtil.getStat(stack, "minersFocusSeconds");
    ...
    private double getBoostPercent(ItemStack stack) {
        double rolledBoost = ProfessionToolUtil.getStat(stack, "minersFocusBoost");
        if (rolledBoost <= 0.0D) {
            return DEFAULT_BOOST_PERCENT;
        }
        return Math.min(300.0D, rolledBoost);
    }
}
