...
public final class GenesisShopMenu {
    ...
    private static void openCategories(ServerPlayer player) {
        ...
        gui.setTitle(Component.literal(GenesisShopConfig.CONFIG.title == null ? "Mega & Special Item Shop" : GenesisShopConfig.CONFIG.title));
        ...
        gui.setSlot(4, new GuiElementBuilder(Items.NETHER_STAR)
                ...
                .addLoreLine(Component.literal("§8Genesis + non-craftable Cobblemon items."))
                .addLoreLine(Component.literal("§8Use categories below to keep the shop clean.")));

        List<String> categories = GenesisShopConfig.categories();
        if (categories.isEmpty()) {
            gui.setSlot(22, new GuiElementBuilder(Items.BARRIER)
                    .hideDefaultTooltip()
                    .setName(Component.literal("§cNo special shop items available"))
                    .addLoreLine(Component.literal("§7Check §fconfig/champutils/genesis_shop.json§7.")));
            gui.open();
            return;
        }
        ...
        for (int i = 0; i < limit; i++) {
            ...
            List<GenesisShopConfig.ShopEntry> entries = GenesisShopConfig.entriesForCategory(category);
    ...
    private static void openCategory(ServerPlayer player, String category, int page) {
        List<GenesisShopConfig.ShopEntry> allEntries = GenesisShopConfig.entriesForCategory(category);
        ...
        List<GenesisShopConfig.ShopEntry> pageEntries = start >= end ? new ArrayList<>() : allEntries.subList(start, end);
        ...
        for (int i = 0; i < pageEntries.size(); i++) {
            GenesisShopConfig.ShopEntry entry = pageEntries.get(i);
            Item icon = GenesisShopService.resolveItem(entry.icon == null || entry.icon.isBlank() ? entry.id : entry.icon);
            if (icon == Items.AIR) icon = GenesisShopService.resolveItem(entry.id);
            if (icon == Items.AIR) icon = Items.BARRIER;

            GuiElementBuilder builder = new GuiElementBuilder(icon)
                    .hideDefaultTooltip()
                    .setName(Component.literal(entry.displayName == null ? "§fGenesis Item" : "§f" + entry.displayName));
            ...
            builder.setCallback((slot, clickType, actionType) -> {
                GenesisShopService.buy(player, entry);
    ...
    private static Item iconFor(List<GenesisShopConfig.ShopEntry> entries) {
        if (entries != null) {
            for (GenesisShopConfig.ShopEntry entry : entries) {
                if (entry == null) continue;
                Item icon = GenesisShopService.resolveItem(entry.icon == null || entry.icon.isBlank() ? entry.id : entry.icon);
                if (icon != Items.AIR) return icon;
                icon = GenesisShopService.resolveItem(entry.id);
                if (icon != Items.AIR) return icon;
            }
        }
