...
public final class GenesisShopConfig {

    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();
    private static final File DIR = new File("config/champutils");
    private static final File FILE = new File(DIR, "genesis_shop.json");

    public static ShopRoot CONFIG = new ShopRoot();

    private GenesisShopConfig() {}
    ...
    private static void sanitize() {
        ...
        for (ShopEntry entry : CONFIG.entries) {
            ...
            if (entry.priceCredits < 0.0D || Double.isNaN(entry.priceCredits) || Double.isInfinite(entry.priceCredits)) entry.priceCredits = defaultPrice(entry.category);
    ...
    private static void addMissingGenesisItems() {
        ...
        for (ResourceLocation id : genesisItems) {
            ...
            entry.priceCredits = defaultPrice(entry.category);
            entry.amount = 1;
            entry.icon = entry.id;
    ...
    private static String niceName(String path) {
        if (path == null || path.isBlank()) return "Genesis Item";
